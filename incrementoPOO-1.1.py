...
class Window:
    ...
    def InsertarGrid(self,elemento): #funcion para ingresar los Widgets a Grid con validacion de input.
        ...
        self.widget.grid(row=row,column=column, pady=5, padx=5,ipadx=5,ipady=5)
    ...
    def sumar(self):
        i=0
        i+=1
        suma= int(self.nombreEntry.get())+(i)
        return self.var.set(suma)

    def crearPushButton(self,nombreButton):
        nombreButton= Button(self.mainWindow,relief="raised", text="+", command= self.sumar)
